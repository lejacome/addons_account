...
class retencion_wizard(osv.osv_memory):
    ...
    _columns = {
                ...
                'lines_ids': fields.one2many('ecua.retencion.wizard.line', 'wizard_id', 'Retention line'),
                }
                ...
                if obj.type in ('in_invoice'):
                    for ret in obj.retention_ids:
                        ...
                        if self._get_automatic(cr, uid, context) and not ret.number_purchase:
                            values['number'] = doc_obj.get_next_value_secuence(cr, uid, 'withholding', False, self._get_printer(cr, uid, context), 'ecua.retencion', 'number_purchase', context)
        ...
        if curr_shop:
            if printer_id:
                ...
                if auth_line_id:
                    ...
                    if automatic:
                        values['automatic_number'] = doc_obj.get_next_value_secuence(cr, uid, 'withholding', False, printer_id, 'ecua.retencion', 'number_purchase', context)
    ...
    def create_retention(self, cr, uid, ids, context=None):
        if not context:
            context = {}
        retention_obj = self.pool.get('ecua.retencion')
        retention_line_obj = self.pool.get('ecua.retencion.line')
        ...
        for retention in self.browse(cr, uid, ids, context):
            ...
            if retention.authorization_sale_id:
                auth_s_obj.check_number_document(cr, uid, retention.number, retention.authorization_sale_id, retention.creation_date, 'ecua.retencion', 'number', _('Withholding') ,context)
    ...
    def approve_now(self, cr, uid, ids, context=None):
        ...
        wf_service.trg_validate(uid, 'ecua.retencion', retention, 'approve_signal', cr)
        ...
        for obj in self.browse(cr, uid, ids, context=None):
            ...
            if obj.type == 'automatic':
                ...
                if not obj.automatic:
                    if not obj.number:
                        raise osv.except_osv(_('Error!'), _('number to be entered to approve the retention'))
                self.pool.get('ecua.retencion').write(cr, uid, [objs.retention_ids[0].id, ], {'number_purchase':obj.number}, context)
                self.pool.get('ecua.retencion').write(cr, uid, [objs.retention_ids[0].id, ], {'automatic': obj.automatic,'creation_date': obj.creation_date, 'authorization_purchase_id': obj.authorization_purchase.id, 'shop_id':obj.shop_id.id, 'printer_id':obj.printer_id.id}, context)
        return {'type': 'ir.actions.act_window_close'}
    
    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        if not context: context = {}
        user = self.pool.get('res.users').browse(cr, uid, uid, context=context)
        res = super(retencion_wizard, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
        valor = context.get('active_ids', [])
        if not valor:
            if view_type == 'form':
...
class retencion_wizard_line(osv.osv_memory):
    _name = "ecua.retencion.wizard.line"
    
    _columns = {
            'wizard_id': fields.many2one('ecua.retencion.wizard', 'wizard'),
