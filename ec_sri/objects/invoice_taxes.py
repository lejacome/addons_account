...
class account_invoice_tax(osv.osv):
    ...
    def compute(self, cr, uid, invoice_id, context=None):
        ...
        for line in inv.invoice_line:
            ...
            #pdb.set_trace()
            if ice:
                for tax in tax_obj.compute_all(cr, uid, taxes, (line.price_unit* (1-(line.discount or 0.0)/100.0)), line.quantity, line.product_id, inv.partner_id)['taxes']:
                ...
                for tax in tax_obj.compute_all(cr, uid, tax_iva, (line.price_unit * (ice_value) * (1-(line.discount or 0.0)/100.0)), line.quantity, line.product_id, inv.partner_id)['taxes']:
                    tax_browse = tax_obj.browse(cr, uid, tax['id'], context)
                    val={}
                    val['invoice_id'] = inv.id
                    val['name'] = tax['name']
