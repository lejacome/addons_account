...
class account_invoice(osv.osv):
    ...
    def onchange_data(self, cr, uid, ids, invoice_number, type=None, shop_id=None, printer_id=None, date=None, context=None):
        ...
        if invoice_number and printer_id:
            #auth = self.pool.get('sri.authorization').check_if_seq_exist(cr, uid, name_document, invoice_number, printer_id, context)
    ...
    def _number(self, cr, uid, ids, name, args, context=None):
        ...
        return result
    def get__partner_address(self, cr, uid, ids,partner_id, context=None):
        values={}
        client = self.pool.get('res.partner').browse(cr, uid, partner_id, context)
        #import pdb
        #pdb.set_trace()
        address="";
        if(client.street):
        ...
        if (client.street2):
            address=address+" & "+client.street2 
        if(client.city):
            address=address+" - "+client.city  
        #values1={"address_invoice":address} 
        values['address_invoice'] = address      
    ...
    def action_number(self, cr, uid, ids, context=None):
        ...
        for invoice in self.browse(cr, uid, ids, context):
            ...
            if invoice.type=='out_invoice':
                #auth_data = self.pool.get('sri.authorization').check_if_seq_exist(cr, uid, 'invoice', invoice.invoice_number_out, invoice.printer_id.id, context)
...
#                wf_service.trg_validate(uid, 'account.invoice', inv_id, 'invoice_open', cr)
    ...
    def onchange_partner_id(self, cr, uid, ids, type, partner_id, date_invoice=False, payment_term=False, partner_bank_id=False, company_id=False):
        ...
        if partner_id:
            ...
            values=self.get__partner_address(cr, uid, ids, partner_id, context={})#by ED.WINTG
            
            res['value']['address_invoice']=values['address_invoice']
            #import pdb
            #pdb.set_trace()
